import {ComponentRef, ComponentFactoryResolver, ApplicationRef, Injector, EmbeddedViewRef, Injectable} from '@angular/core';
...
export abstract class BaseService {
   ...
   * 销毁
   *
   * @param {number | ComponentRef<any>} [component] 下标（从0开始或组件引用对象），或不指定时，销毁最新一个
   */
  destroy(component?: number | ComponentRef<any>) {
    if (typeof component === 'number') {
      component = this.list[<number>component];
    }
    if (!component) {
      component = this.list.pop();
    }
    if (component) {
      (<ComponentRef<any>>component).destroy();
    }
  }
